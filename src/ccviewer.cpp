...
void CCViewer::resizeGL() {
  ...
  float aspect = w / h;
  const qreal zNear = 0, zFar = 10.0, fov = 70.0;
  projection.setToIdentity();
  projection.perspective(fov, aspect, zNear, zFar);
  projection.lookAt(QVector3D(10, 10, 10), QVector3D(.5, .5, 0),
                    QVector3D(0, 1, 0));
}
...
void CCViewer::paint() {
  ...
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo.e);
  glDrawElements(GL_LINES, raw->n2 * 2, GL_UNSIGNED_INT, nullptr);

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vbo.f);
  glDrawElements(GL_TRIANGLES, raw->n3 * 3, GL_UNSIGNED_INT, nullptr);
